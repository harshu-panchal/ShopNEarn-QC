...
import {
  getMlmDailyCapRolloverJobHandler,
  getMlmDailyCapRolloverJobIntervalMs,
  isMlmDailyCapRolloverJobEnabled,
} from "./app/jobs/mlmDailyCapRolloverJob.js";
import {
  getMlmWalletLedgerVerifierJobHandler,
  getMlmWalletLedgerVerifierJobIntervalMs,
...
async function startScheduler() {
  ...
  // MLM Phase 3 — daily cap rollover. Runs hourly so the IST midnight
  // crossover is caught within an hour. Idempotent; toggle via
  // ENABLE_MLM_DAILY_CAP_ROLLOVER_JOB (default true).
  if (isMlmDailyCapRolloverJobEnabled()) {
    registerScheduledJob(
      'mlmDailyCapRolloverJob',
      getMlmDailyCapRolloverJobIntervalMs(),
      getMlmDailyCapRolloverJobHandler()
    );
  }

  // MLM Phase 5 — wallet ↔ ledger drift verifier (read-only). OFF by
  // default; flip ENABLE_MLM_WALLET_LEDGER_VERIFIER_JOB=true to enable.
  if (isMlmWalletLedgerVerifierJobEnabled()) {
    registerScheduledJob(
      'mlmWalletLedgerVerifierJob',
      getMlmWalletLedgerVerifierJobIntervalMs(),
      getMlmWalletLedgerVerifierJobHandler()
    );
  }
