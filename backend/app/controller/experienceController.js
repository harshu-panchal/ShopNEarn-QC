...
// Maximum products we hydrate per "products" experience section in a single
// response. The customer renderer lazy-loads from this list, so this is the
// soft ceiling on the section's visible products (not a layout dimension).
const MAX_PRODUCTS_PER_SECTION = 60;
...
export const getPublicExperienceSections = async (req, res) => {
  try {
    ...
    // Hydrate matched products for any "products" section so the customer app
    // does not have to know how to translate the section filter (categoryIds /
    // subcategoryIds / explicit productIds) into a product query.
    const hydrated = await hydrateMatchedProductsForSections(sections, req);

    return handleResponse(res, 200, "Experience sections fetched", hydrated);
  } catch (error) {
    return handleResponse(res, 500, error.message);
  }
};
...
/* ===============================
   Helpers: hydrate products on sections
================================ */
async function hydrateMatchedProductsForSections(sections, req) {
  if (!Array.isArray(sections) || !sections.length) return sections || [];

  const productSections = sections.filter(
    (s) => s?.displayType === "products" && s?.config?.products,
  );
  ...
  const enrichedSections = await Promise.all(
    sections.map(async (section) => {
      if (section?.displayType !== "products") return section;
      const productConfig = section?.config?.products || {};
      const productIds = (productConfig.productIds || []).map(String);
      const categoryIds = (productConfig.categoryIds || []).map(String);
      const subcategoryIds = (productConfig.subcategoryIds || []).map(String);

      const matched = await queryMatchedProducts({
        productIds,
        categoryIds,
        subcategoryIds,
        allowedSellerIds,
        applyCustomerFilters: isCustomer,
      });

      return { ...section, matchedProducts: matched };
    }),
  );
...
async function queryMatchedProducts({
  productIds,
  categoryIds,
  subcategoryIds,
  allowedSellerIds,
  ...
  const hasAnyFilter =
    productIds.length || categoryIds.length || subcategoryIds.length;
  ...
  if (productIds.length) {
    ...
    if (subcategoryIds.length) filter.subcategoryId = { $in: subcategoryIds };
  }

  if (Array.isArray(allowedSellerIds)) {
    filter.sellerId = { $in: allowedSellerIds };
  }

  let finalFilter = filter;
  if (applyCustomerFilters) {
    finalFilter = { $and: [{ ...filter, status: "active" }, getApprovedOrLegacyFilter()] };
  }

  // Select only the fields the customer ProductCard needs (keeps payload lean
  // even when 60 products per section are returned).
  const selectFields = [
    "name",
    "slug",
    "price",
  ...
  return Product.find(finalFilter)
    ...
    .limit(MAX_PRODUCTS_PER_SECTION)
    .lean();
}
