...
async function settleDeliveredOrderInSession(orderOrId, { actorId = null, session }) {
    ...
    // MLM Phase 2: Plan B repurchase bonus chain + (Phase 4) Home
    // Shopping commissions fire AFTER the seller / rider / admin
    // payouts have been recorded but BEFORE the final `order.save`.
    // Both are session-bound and idempotent; failures here MUST roll
    // back the entire delivered-settlement because the customer's
    // downline must agree with the order's accounting state.
    if (!order.mlmBonusesDisbursed) {
      try {
        ...
        await computeAndCreditRepurchaseBonusChain({
          orderId: order._id,
          downlineUserId: order.customer,
          session,
          correlationId: `SETTLE-${order._id}`,
        });
      } catch (error) {
        if (typeof console !== "undefined" && console.warn) {
          console.warn("[orderFinanceService] MLM repurchase chain failed during settlement; aborting", {
      ...
      // Phase 4 Home Shopping commissions
      if (order.isHomeShoppingOrder) {
        try {
          const { computeAndCreditHomeShoppingCommissions } = await import("../mlm/mlmBonusEngineService.js");
          if (typeof computeAndCreditHomeShoppingCommissions === "function") {
            await computeAndCreditHomeShoppingCommissions({
              orderId: order._id,
              downlineUserId: order.customer,
              session,
              correlationId: `SETTLE-HS-${order._id}`,
            });
          }
        } catch (error) {
          if (typeof console !== "undefined" && console.warn) {
            console.warn("[orderFinanceService] MLM Home Shopping commissions failed during settlement", {
              orderId: String(order._id),
              error: error.message,
            });
          }
