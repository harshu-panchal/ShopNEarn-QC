    ...
    try {
      ...
      const payload = {
        ...
        premiumUpgradeShoppingWalletTopup:
          Number(cfg.premiumUpgradeShoppingWalletTopup) || 0,
        planBAutoUpgradeAtPlanALifetimeEarnings:
          Number(cfg.planBAutoUpgradeAtPlanALifetimeEarnings) || 0,
        ...
        binaryTopupPairIncome: {
          ...
          eligibilityLifetimeEarnings:
            Number(cfg.binaryTopupPairIncome?.eligibilityLifetimeEarnings) ||
            30000,
        ...
        planAPairBonusReleaseCooldownDays:
          Number(cfg.planAPairBonusReleaseCooldownDays) || 0,
        ...
        homeShoppingProductCreditValue:
          Number(cfg.homeShoppingProductCreditValue) || 0,
        homeShoppingCommissions: {
          salesPercent: Number(cfg.homeShoppingCommissions?.salesPercent) || 0,
          referralPercent:
            Number(cfg.homeShoppingCommissions?.referralPercent) || 0,
          royaltyPercent:
            Number(cfg.homeShoppingCommissions?.royaltyPercent) || 0,
        },
        withdrawalMinAmount: Number(cfg.withdrawalMinAmount) || 0,
        withdrawalAdminChargePercent:
          Number(cfg.withdrawalAdminChargePercent) || 0,
        withdrawalGstOnAdminChargePercent:
          Number(cfg.withdrawalGstOnAdminChargePercent) || 0,
        ...
        directReferralActivationBonusEnabled:
          cfg.directReferralActivationBonusEnabled !== false,
        directReferralFirstPairEnabled:
          cfg.directReferralFirstPairEnabled !== false,
        directReferralFirstPairAmount:
          Number(
            cfg.directReferralFirstPairAmount ??
              cfg.directReferralActivationSponsorAmount,
          ) || 200,
        directReferralPerActivationEnabled:
          cfg.directReferralPerActivationEnabled !== false,
        directReferralPerActivationAmount:
          Number(
            cfg.directReferralPerActivationAmount ??
              cfg.directReferralActivationSponsorAmount,
          ) || 200,
        directReferralActivationSponsorAmount:
          Number(cfg.directReferralActivationSponsorAmount) || 200,
      };
    ...
    <div className="p-4 sm:p-6 space-y-6 max-w-4xl">
      ...
      <Section title="Program Toggle">
        ...
        <p className="text-xs text-slate-500 mt-2">
          When disabled, signup referral codes are still captured but no bonuses
          fire and the customer dashboard shows a "coming soon" placeholder.
        </p>
        <div className="mt-4 pt-4 border-t border-slate-200">
          ...
          <p className="text-xs text-slate-500 mt-2 leading-relaxed">
            When ON, no new customer can create an account without a valid
            sponsor referral code. The signup OTP endpoint rejects the request
      ...
      <Section title="Plan A Activation">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <NumField
            ...
            value={cfg.joiningPackagePrice}
            onChange={(v) => setCfg({ ...cfg, joiningPackagePrice: v })}
          />
          <NumField
            label="Shopping wallet credit on Plan A activation (₹)"
            value={cfg.joiningPackageShoppingWalletCredit}
            onChange={(v) =>
              setCfg({ ...cfg, joiningPackageShoppingWalletCredit: v })
            }
          />
        </div>
        <p className="text-[11px] text-slate-500 mt-2 leading-relaxed">
          Plan A Activation is a direct payment subscription — no product or
          order is created. The price and shopping credit are snapshotted when
          the customer clicks "Join Now", so mid-flight edits never cheat
          in-flight customers.
        </p>
      </Section>
      ...
      <Section title="Direct Referral Activation Income (earnings)">
        <Toggle
          ...
          onChange={(v) =>
            setCfg({ ...cfg, directReferralActivationBonusEnabled: v })
          }
        />
        <p className="text-[11px] text-slate-500 mt-2 mb-4 leading-relaxed">
          Two independent flows can run together when a direct referral
          activates Plan A. Both credit the sponsor&apos;s{" "}
          <strong>earnings</strong> wallet.
        </p>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-3">
            <Toggle
              ...
              onChange={(v) =>
                setCfg({ ...cfg, directReferralFirstPairEnabled: v })
              }
            />
            <NumField
              label="Amount (₹)"
              ...
              onChange={(v) =>
                setCfg({ ...cfg, directReferralFirstPairAmount: v })
              }
            />
          </div>
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-3">
            <Toggle
              ...
              onChange={(v) =>
                setCfg({ ...cfg, directReferralPerActivationEnabled: v })
              }
            />
            <NumField
              label="Amount (₹)"
              ...
              onChange={(v) =>
                setCfg({ ...cfg, directReferralPerActivationAmount: v })
              }
            />
          </div>
        </div>
      ...
      <Section title="Plan A: Binary Pair Income (team matching)">
        <p className="text-[11px] text-slate-500 leading-relaxed mb-3">
          Matches client PHP spec: first pair 2:1 or 1:2, then 1:1 on active
          Plan A volume in left vs right team legs. Income per pair and daily
          pair cap depend on the sponsor&apos;s active Plan A direct count. Only
          active Plan A IDs in each subtree count.
        </p>
        <RuleEditor
        ...
        />
        <p className="text-[11px] font-semibold text-slate-600 mt-4 mb-2">
          Topup member override
        </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-4">
          <NumField
            ...
            onChange={(v) =>
              setCfg({ ...cfg, planAPairBonusReleaseCooldownDays: v })
            }
            min={0}
            max={365}
          />
      ...
      <Section title="Home Shopping (Plan B exclusive)">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          <TextField
            label="Product ID"
            value={cfg.homeShoppingProductId || ""}
            onChange={(v) =>
              setCfg({ ...cfg, homeShoppingProductId: v.trim() || null })
            }
          />
          <NumField
            label="Price (₹)"
            value={cfg.homeShoppingPrice}
            onChange={(v) => setCfg({ ...cfg, homeShoppingPrice: v })}
          />
          <NumField
            label="Product credit value (₹)"
            value={cfg.homeShoppingProductCreditValue}
            onChange={(v) =>
              setCfg({ ...cfg, homeShoppingProductCreditValue: v })
            }
          />
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-2">
          <NumField
            label="Sales %"
            value={cfg.homeShoppingCommissions?.salesPercent || 0}
            onChange={(v) =>
              setCfg({
                ...cfg,
                homeShoppingCommissions: {
                  ...cfg.homeShoppingCommissions,
                  salesPercent: v,
                },
              })
            }
          ...
          <NumField
            label="Referral %"
            value={cfg.homeShoppingCommissions?.referralPercent || 0}
            onChange={(v) =>
              setCfg({
                ...cfg,
                homeShoppingCommissions: {
          ...
          <NumField
            label="Royalty %"
            value={cfg.homeShoppingCommissions?.royaltyPercent || 0}
            onChange={(v) =>
              setCfg({
                ...cfg,
                homeShoppingCommissions: {
      ...
      <Section title="Withdrawals">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <NumField
            ...
            value={cfg.withdrawalMinAmount}
            onChange={(v) => setCfg({ ...cfg, withdrawalMinAmount: v })}
          />
          <NumField
            label="Admin charge %"
            value={cfg.withdrawalAdminChargePercent}
            onChange={(v) =>
              setCfg({ ...cfg, withdrawalAdminChargePercent: v })
            }
            max={100}
          />
          <NumField
            label="GST / statutory %"
            value={cfg.withdrawalGstOnAdminChargePercent}
            onChange={(v) =>
              setCfg({ ...cfg, withdrawalGstOnAdminChargePercent: v })
            }
            max={100}
          />
        </div>
      ...
      <Section title="Cap & Behaviour">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <NumField
            ...
            value={cfg.dailyEarningCap}
            onChange={(v) => setCfg({ ...cfg, dailyEarningCap: v })}
          />
          <SelectField
            ...
            options={[
              {
                value: "spillover",
                label: "Spillover — leg choice locked (recommended)",
              },
              {
                value: "manual",
                label: "Manual — same as spillover when leg chosen",
              },
              {
                value: "balanced_auto",
                label: "Balanced — weaker leg (legacy only, no leg link)",
              },
            ]}
          />
          <SelectField
          ...
          />
          <NumField
            label="Sponsor chain max depth"
            value={cfg.sponsorChainMaxDepth}
            onChange={(v) => setCfg({ ...cfg, sponsorChainMaxDepth: v })}
            min={1}
            max={50}
          />
        </div>
      ...
      <Section title="Maintenance Tools">
        <p className="text-xs text-slate-500 mb-4 leading-relaxed">
          Run backend maintenance scripts to repair the binary tree, refresh
          counters, and backfill or recalculate member incomes. Preview runs are
          safe (no writes); Apply commits changes to the database.
        </p>
        <MlmMaintenanceTools />
      </Section>
    </div>
...
const BinaryPairIncomePreview = ({ cfg }) => {
  ...
  const tierFor5 =
    tiers.find((t) => Number(t.minDirectCount) <= 5) || tiers[tiers.length - 1];
  ...
  return (
    <div className="mt-4 bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs text-slate-600 space-y-1">
      ...
      <div>
        Left 15 · Right 10 · 5 directs → <strong>{pairs} pairs</strong> (cap{" "}
        {dailyCap})
      </div>
      <div>
        Income: <strong>₹{income.toLocaleString("en-IN")}</strong> ({capped} × ₹
        {pairIncome})
      </div>
      <div>
        Balance: left {leftBal}, right {rightBal}
      </div>
    </div>
  );
};
    ...
    <div className="mt-4 bg-slate-50 border border-slate-200 rounded-xl p-3">
      ...
      <div className="flex flex-wrap gap-2">
        {Array.from({ length: previewMax }, (_, i) => i + 1).map((idx) => {
          ...
          const isFixed =
            idx > fixedAfter &&
            !Object.prototype.hasOwnProperty.call(tiers, idx);
          return (
            <div
              key={idx}
      ...
      <div className="text-[10px] text-slate-500 mt-2">
        Indigo = explicit tier row · Emerald = fixed-amount fallback (pairs &gt;{" "}
        {fixedAfter})
      </div>
    </div>
  );
                ...
                <div className="flex gap-3 items-start">
                  ...
                  <div className="flex-1 space-y-2">
                    ...
                    <p className="text-[11px] text-slate-500">
                      Empty image falls back to a bundled placeholder on the
                      customer page.
                    </p>
                  </div>
                </div>
