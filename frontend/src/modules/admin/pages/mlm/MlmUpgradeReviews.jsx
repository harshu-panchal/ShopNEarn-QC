...
const MlmUpgradeReviews = () => {
  ...
  const load = async () => {
    ...
    try {
      ...
      const res = await adminMlmApi.listUpgradeReviews(params);
  ...
  const handleApprove = async () => {
    ...
    if (
      !window.confirm(
        `Approve Plan B upgrade for ${selected.customer?.name || selected.customer?.phone || "this customer"}?\n\nAmount: ${formatINR(selected.amount)}\nTransaction ID: ${selected.transactionId || "—"}`,
      )
    ) {
      return;
    }
    ...
    try {
      await adminMlmApi.approveUpgradeReview(selected._id, {
        adminRemarks: adminRemarks.trim() || undefined,
      });
      toast.success("Approved — Plan B upgrade applied");
      closeRow();
      await load();
    } catch (err) {
  ...
  const handleReject = async () => {
    ...
    setActionId(selected._id);
    try {
      await adminMlmApi.rejectUpgradeReview(selected._id, { reason });
      ...
      <div className="flex flex-wrap items-start sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-slate-900">
            MLM Plan B Upgrade Reviews
          </h1>
          <p className="text-xs text-slate-500 mt-1">
            Manual QR payments for opt-in Plan B upgrades.
          </p>
        </div>
...
export default MlmUpgradeReviews;
