...
const RightTeamPage = () => {
    ...
    useEffect(() => {
        ...
        (async () => {
            setLoading(true);
            try {
                const res = await mlmApi.getLegTeam({ leg: 'R', page, limit, filter: filter !== 'ALL' ? filter : undefined });
        ...
        <div className="min-h-screen bg-slate-50 pb-24 md:pb-12">
            ...
            <div className="max-w-7xl mx-auto px-4 md:px-8 space-y-4 md:space-y-6">
                <div className="hidden md:flex items-end justify-between gap-4 pt-6 pb-4 border-b border-slate-200/80">
                    <div>
                        <h1 className="text-2xl font-black text-slate-900 tracking-tight">Right Team</h1>
                        <p className="text-sm text-slate-500 mt-1">View members placed in your right leg.</p>
                    </div>
                </div>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left text-sm whitespace-nowrap">
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {loading ? (
                                    <tr>
                                        <td colSpan={3} className="px-5 py-12 text-center text-slate-500">Loading team...</td>
                                    </tr>
                                ) : team.length === 0 ? (
                                    <tr>
                                        <td colSpan={3} className="px-5 py-12 text-center text-slate-500">No members found in right leg.</td>
                                    </tr>
                                ) : (
                                    team.map((row) => (
