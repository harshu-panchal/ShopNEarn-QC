...
const statusBadge = {
    ...
    approved: { label: 'Approved', icon: CheckCircle2, color: 'bg-blue-50 text-blue-700 border-blue-200' },
    paid: { label: 'Paid', icon: CheckCircle2, color: 'bg-emerald-50 text-emerald-700 border-emerald-200' },
    rejected: { label: 'Rejected', icon: XCircle, color: 'bg-rose-50 text-rose-700 border-rose-200' },
    cancelled: { label: 'Cancelled', icon: XCircle, color: 'bg-slate-100 text-slate-600 border-slate-200' },
};

const MlmWithdrawalPage = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (form.method === 'upi') {
            ...
            if (!form.ifsc.trim()) { toast.error('IFSC is required'); return; }
            ...
            <div className="max-w-2xl mx-auto px-4 space-y-4">
                ...
                <div className="bg-gradient-to-br from-violet-600 to-indigo-700 text-white rounded-2xl p-5">
                    ...
                    <p className="text-xs opacity-80 mt-2">
                        Min withdrawal {formatINR(minAmt)} · Fees deducted: {membership.config.withdrawalAdminChargePercent}% admin + {membership.config.withdrawalGstOnAdminChargePercent}% GST on charge
                    </p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
                    ...
                    {preview && (
                        <div className="bg-slate-50 rounded-xl p-3 text-xs space-y-1.5">
                            ...
                            <div className="flex justify-between text-rose-600"><span>GST on charge</span><span className="font-bold">-{formatINR(preview.gst)}</span></div>
                    ...
                    <div>
                        <p className="text-xs font-bold uppercase tracking-wide text-slate-600 mb-2">Method</p>
                                    ...
                                    <li key={r._id} className="px-5 py-3">
                                        <div className="flex items-start justify-between">
                                            <div className="flex items-start gap-3">
                                                ...
                                                <div>
                                                    <p className="text-sm font-bold text-slate-900">{formatINR(r.amount)}</p>
                                                    <p className="text-[11px] text-slate-500">Net: {formatINR(r.netPayoutAmount)} · {formatDate(r.createdAt)}</p>
                                                    {r.payoutReference && (
                                                        <p className="text-[10px] text-emerald-700 mt-0.5">Ref: {r.payoutReference}</p>
                                                    )}
                                                    {r.rejectionReason && (
                                            ...
                                            <div className="flex flex-col items-end gap-2">
                                                <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full border text-[10px] font-bold uppercase tracking-wide ${badge.color}`}>
...
const Header = ({ navigate }) => (
    <div className="sticky top-0 z-30 bg-slate-50/95 backdrop-blur-sm px-4 pt-4 pb-3 border-b border-slate-200/60 mb-4 flex items-center gap-2">
        ...
        <h1 className="text-xl font-semibold text-slate-900 tracking-tight">Withdraw</h1>
    </div>
);

export default MlmWithdrawalPage;
