 ...
 *
 * Tab isolation:
 *   - The customer JWT is stored under `auth_customer` in
 *     localStorage, separate from the admin's `auth_admin` token.
 *     So the admin's existing tab remains authenticated as admin
 *     while this new tab runs as the impersonated customer.
 ...
 */
const ALLOWED_REDIRECT_PREFIXES = ['/mlm', '/profile', '/orders', '/wallet', '/'];
...
const parseHashParams = () => {
    ...
    return {
        ...
        redirect: params.get('redirect') || '/mlm',
    };
};
...
const AuthHandoff = () => {
    ...
    useEffect(() => {
        const { token, redirect } = parseHashParams();
        ...
        const target = isSafeRedirect(redirect) ? redirect : '/mlm';
